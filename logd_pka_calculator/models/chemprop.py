...
import numpy as np
import chemprop
from logd_pka_calculator.data.make_dataset import (
    chemprop_singletask_data,
    chemprop_multitask_data,
)
...
def multi_task_model_train() -> None:
    """Trains a multi-task chemprop model for logD.
    Includes chemprop RDKit 2D descriptors.
    Saves the model in models/
    """
    chemprop_multitask_data()
    ...
    arguments = [
        "--data_path",
        str(mdf("data/processed/train_data_chemprop_multitask.csv")()),
        "--dataset_type",
        "regression",
        "--save_dir",
        str(mdf("models/chemprop_multitask")()),
        "--features_generator",
        "rdkit_2d_normalized",
        "--no_features_scaling",
        "--split_type",
...
def multi_task_model_predict(smiles: list) -> np.array:
    """Uses the chemprop model to make predictions on test data.
